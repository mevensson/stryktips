...
- For `--week` and `--start-week`, omitting `.N` is the same as `.1`. A week spanning two months
  ...
  With `--start-week`, the selected draw is the inclusive report start, so `.2`
  excludes the first draw of that week from the report.
...
- Omitting the end-week index differs from explicitly supplying `.1`. For the
  ...
  Starting at draw 4881 instead includes only that second draw with
  `--end-week 2024.52` or `2024.52.2`; `2024.52.1` resolves before the start
  and exits 1 with an ordering error before fetching any report draws.
