...
The Test-Driven Development (TDD) Workflow is used to step by step implement a feature, by first writing a failing test (red), then making it pass (green), and finally refactor.
...
- When the test has been implemented and fails as expected, the sub agent shall finish by creating a commit with the changes. The commit message shall start with "Red: ".
...
Decide which findings should be fixed using the rules below, and create the **triaged review findings** with the findings that should be fixed now.

Rules:
- No new functionality.
- No bug fixes:
  - Bugs introduced in the current session shall be added to **end goal**, and fixed in another iteration.
  - Preexisting bugs shall be added to **preexisting bugs list**.
- Technical debt should in general be fixed, especially if it's large and/or crosscutting.
- Don't reject a finding just because it's "preexisting / not touched by this branch / out of scope".

### Handle the review comments

Delegate to the `implementer` subagent to handle the **triaged review findings**.

**Sub agent prompt**
- The **triaged review findings**.
- All findings must be fixed.
- A separate commit shall be made for each finding. The commit messages shall start with "Refactor: ".

## Step 5 - Check if **end goal** has been reached

Evaluate if the **end goal** has been reached. If the **end goal** has not been reached, do another iteration of Step 1 to 4.

If the **end goal** has been reached:
* Summarize what has been done.
* Show **preexisting bugs list** and ask the user what to do with it.
